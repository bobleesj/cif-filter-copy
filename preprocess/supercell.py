...
def calculate_distance(point1, point2, cell_lengths, angles):
    ...
    delta_x2, delta_y2, delta_z2, label2 = list(map(float, point2[:-1])) + [
        point2[-1]
    ]
    ...
    for i, point1 in enumerate(flattened_points):
        ...
        for j, point2 in enumerate(flattened_points):
            if i != j:
                pair = tuple(
                    sorted([i, j])
                )  # Sort the pair so (i, j) is treated as equivalent to (j, i)
                if (
                    pair not in pairs_set
                    ...
                    if (
                        abs(distance) > 1e-3
                    ):  # Update the condition with the tolerance value
