...
from core.filter import min_distance
...
def get_cif_folder_info(cif_dir_path, is_interactive_mode=True):
    ...
    cif_file_list = folder.get_cif_file_paths_from_directory(cif_dir_path)
    ...
    for i, file_path in enumerate(cif_file_list, start=1):
        ...
        prompt.print_progress_current(
            i, filename_base, supercell_points, len(cif_file_list)
        )
