...
def test_move_files_based_on_supercell_size():
    ...
    filtered_dir = join(base_dir, "supercell_size_cif_files_filter_supercell_size")
    ...
    assert get_cif_file_count_from_directory(filtered_dir) == 4, "Not all expected files were copied."
    assert get_cif_file_count_from_directory(base_dir) == 1, "Not all expected files were copied."
    ...
    assert get_cif_file_count_from_directory(base_dir) == 5, "Not all expected files were copied."
    remove_directories([filtered_dir])
