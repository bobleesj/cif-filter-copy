...
def move_files_based_on_min_dist(cif_dir, isInteractiveMode=True):
    ...
    for i, file_path in enumerate(file_path_list):
        ...
        for site_label, pair_data in all_labels_connections.items():
            ...
            if dist < min_dist:
                min_dist = dist  # Update min_dist if a new smaller distance is found
    ...
    # Filter files based on the minimum distance threshold
    filtered_files = [file for file, dist in paired_list if dist < min_dist_threshold]

    # Check if there are any files to process
    if filtered_files:
        # Create a new directory to store files that meet the condition
        new_folder_path = os.path.join(cif_dir, f"min_dist_below_{min_dist_threshold}")
        ...
        df = pd.DataFrame({'files': file_path_list, 'dist': file_min_dist_list})

        # Define the folder path where the CSV will be saved

        # Define a base filename for the CSV file
        base_filename = "min_dist_log"

        # Call the function to save the DataFrame as a CSV in the specified folder
        folder.save_to_csv_directory(cif_dir, df, base_filename)


    else:
        print("No files found with min distance below the threshold.")